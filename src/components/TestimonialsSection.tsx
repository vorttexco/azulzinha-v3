  ...
  return (
    <section className="bg-white py-14 lg:py-20 overflow-hidden">
      <div className="max-w-[1440px] mx-auto">
        <h2 className="text-center text-[26px] lg:text-[38px] leading-[1.2] lg:leading-[1.3] font-bold text-[#006CAD] px-[30px] lg:px-4">
          <span className="text-[#FC8F01]">Histórias</span> reais de quem já é
          azulzinha
        </h2>

        <div className="mt-8 lg:mt-[60px] flex items-stretch gap-6 px-[30px] lg:px-6 lg:justify-center overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className={`w-[280px] lg:w-[350px] shrink-0 bg-white rounded-xl p-6 lg:p-8 flex flex-col justify-between shadow-[0_4px_10px_rgba(0,0,0,0.08)] ${testimonial.opacity}`}
            >
              <p className="text-sm lg:text-base italic text-black leading-relaxed">
              ...
              <div className="mt-6">
                <p className="font-bold text-[#006CAD] text-sm lg:text-base">
                  {testimonial.author}
                </p>
                <p className="text-black text-sm lg:text-base">{testimonial.business}</p>
              </div>
            </div>
          ))}
        </div>
